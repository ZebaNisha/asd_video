#!/usr/bin/env python
"""Train fine‑tuned VGG16 + BiLSTM for Phase 1.5 validation.

Output directory: c:/asd_project/outputs/finetune_vgg16_phase15
"""
...
# --------------------------------------------------------------------
# Paths (adjusted for Phase 1.5)
# --------------------------------------------------------------------
BASE_DIR = pathlib.Path('c:/asd_project')
OUTPUT_DIR = BASE_DIR / 'outputs' / 'finetune_vgg16_phase15'
...
# --------------------------------------------------------------------
# Data loading utilities – same as pilot but pointing to Phase 1.5 folders
# --------------------------------------------------------------------
def load_split(split: str) -> Tuple[np.ndarray, np.ndarray]:
    """Load image sequences and labels for a given split.
    Expected layout: crops/<split>/*.jpg where filename contains video_id.
    """
    ...
    for video_id, frames in video_groups.items():
        ...
        # Infer label from subject mapping (subject id is prefix of video_id)
        label = 0
...
# --------------------------------------------------------------------
# Model definition – identical to pilot
# --------------------------------------------------------------------
def build_model():
    ...
    bilstm = tf.keras.layers.Bidirectional(tf.keras.layers.LSTM(32), name='bilstm')(td)
    dense1 = tf.keras.layers.Dense(64, activation='relu', name='dense1')(bilstm)
    drop = tf.keras.layers.Dropout(0.5, name='dropout')(dense1)
    out = tf.keras.layers.Dense(1, activation='sigmoid', name='out')(drop)
    return tf.keras.Model(inputs=input_seq, outputs=out)
...
def train_and_evaluate():
    ...
    pred_df = pd.DataFrame({'true_label': y_test, 'pred_prob': probs, 'pred_label': preds})
    pred_df.to_csv(OUTPUT_DIR / 'predictions.csv', index=False)
    # Training log CSV
    pd.DataFrame(history.history).to_csv(OUTPUT_DIR / 'training_log.csv', index_label='epoch')
    # Report JSON
    report = {
        'train_metrics': {
        ...
        'success': {
            'roc_auc_threshold_met': roc >= 0.60
        }
    }
...
"""
    (OUTPUT_DIR / 'comparison.md').write_text(comp_md, encoding='utf-8')
    # Recommendation
    if roc >= 0.60:
        recommendation = "Pilot result appears genuine. Proceed to larger Phase 2 experiment."
    else:
        recommendation = "Pilot result likely noise. Stop scaling VGG16 experiments."
    rec_path = REPORTS_DIR / 'recommendation.md'
